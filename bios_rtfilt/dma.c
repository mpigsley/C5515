...
int16_t filteredSig[DMA_PING_PONG_BUFFER_LENGTH * 2];
int nextFilteredSig = 0;
int16_t volumeControl = 26000;
...
void DmaInterrupt(void) {
    ...
    if ((DMAIFR & BIT0) == BIT0) {
        ...
        if (DMACH0TCR2 & BIT1) {
        	...
        	for (i = 0; i < DMA_PING_PONG_BUFFER_LENGTH; i++) {
        		// Perform Volume Control Sample-by-Sample
        		int16_t val = (int16_t) ((((int32_t) filteredSig[i + (nextFilteredSig * DMA_PING_PONG_BUFFER_LENGTH)]) * volumeControl) >> 16);

        		// Write to Output Buffer
        		g_dmaOutputBuffer[i + DMA_PING_PONG_BUFFER_LENGTH] = ((int32_t) val) << 16;
        	}
        }  else {
        	...
        	for (i = 0; i < DMA_PING_PONG_BUFFER_LENGTH; i++) {
        		...
        		// Write to Output Buffer
        		g_dmaOutputBuffer[i] = ((int32_t) val) << 16;
        	}
        }
    ...
    if ((DMAIFR & BIT1) == BIT1) {
    	...
    	int16_t tempBuffer[DMA_PING_PONG_BUFFER_LENGTH]; // Create temporary buffer for bit shifted samples

        if (DMACH1TCR2 & BIT1) { // The receive "pong" buffer is full
        	// Convert Input Buffer into 16bit
        	for (i = 0; i < DMA_PING_PONG_BUFFER_LENGTH; i++)
        		tempBuffer[i] = (int16_t) (g_dmaInputBuffer[i + DMA_PING_PONG_BUFFER_LENGTH] >> 16);

        	// Filter
        	int16_t* val = filterBuffer(tempBuffer);
        	memcpy(&filteredSig[DMA_PING_PONG_BUFFER_LENGTH],
        			&val[0],
        			DMA_PING_PONG_BUFFER_LENGTH * sizeof(filteredSig[0]));

            nextFilteredSig = 1;
        } else { // The receive "ping" buffer is full
        	// Convert Input Buffer into 16bit
        	for (i = 0; i < DMA_PING_PONG_BUFFER_LENGTH; i++)
        		tempBuffer[i] = (int16_t) (g_dmaInputBuffer[i] >> 16);

        	// Filter
        	int16_t* val = filterBuffer(tempBuffer);
        	memcpy(&filteredSig[0],
        			&val[0],
        			DMA_PING_PONG_BUFFER_LENGTH * sizeof(filteredSig[0]));
